...
func preparePodFlows(podIPs []net.IP, podMAC net.HardwareAddr, podOFPort uint32, gwMAC, vMAC net.HardwareAddr, nodeConfig *config1.NodeConfig, connectUplinkToBridge bool, vlanID uint16) []expectTableFlows {
	...
	if isAntreaFlexibleIPAM {
		flows = append(flows, []expectTableFlows{{
			...
			[]*ofTestUtils.ExpectFlow{
				{
					MatchStr: fmt.Sprintf("priority=200,in_port=%d%s,dl_dst=%s", 3, matchVlanVIDString, podMAC.String()),
					ActStr:   fmt.Sprintf("load:0x1->NXM_NX_REG8[12..15],load:%s->NXM_NX_REG8[0..11],load:0x3->NXM_NX_REG0[0..3],goto_table:SNATConntrackZone", vlanVIDString),
				},
			},
		}}...)
		if vlanID == 0 {
			flows = append(flows, []expectTableFlows{{
				...
				[]*ofTestUtils.ExpectFlow{
					{
						MatchStr: fmt.Sprintf("priority=200,in_port=LOCAL,vlan_tci=0x0000/0x1fff,dl_dst=%s", podMAC.String()),
						ActStr:   fmt.Sprintf("load:0x1->NXM_NX_REG8[12..15],load:0x4->NXM_NX_REG0[0..3],goto_table:SNATConntrackZone"),
					},
				},
			}}...)
		} else {
				...
				[]*ofTestUtils.ExpectFlow{
					{
						...
						ActStr:   fmt.Sprintf("push_vlan:0x8100,set_field:%d->vlan_vid,goto_table:L2ForwardingOut", vlanID+4096),
					},
				},
			}}...)
		}
				...
				[]*ofTestUtils.ExpectFlow{
					{
						...
						ActStr:   fmt.Sprintf("%sresubmit(,%s)", actionSetCtZoneField, nextTableForSpoofguard),
					},
				},
			},
			expectTableFlows{
				...
				[]*ofTestUtils.ExpectFlow{
					{
						...
						ActStr:   fmt.Sprintf("%sset_field:%s->eth_dst,resubmit(,L3DecTTL)", actionNotAntreaFlexibleIPAMString, podMAC.String()),
					},
				},
			},
		)
	...
	for _, gwIP := range gwIPs {
					...
					[]*ofTestUtils.ExpectFlow{
						{
							...
							ActStr:   fmt.Sprintf("%sresubmit(,SNATConntrackZone)", actionSetCtZoneField),
						},
					},
				},
				expectTableFlows{
		...
		flows = append(flows,
			...
			expectTableFlows{
				...
				[]*ofTestUtils.ExpectFlow{
					{
						MatchStr: fmt.Sprintf("priority=210,ct_state=+rpl+trk,ct_mark=0x1/0xf,%s", ipProtoStr),
						ActStr:   fmt.Sprintf("set_field:%s->eth_dst,load:0x1->NXM_NX_REG0[4..7],resubmit(,L3DecTTL)", gwMAC.String()),
					},
					{
			...
			[]*ofTestUtils.ExpectFlow{
				{
					...
					ActStr:   "load:0->NXM_NX_REG0[0..3],load:0x1->NXM_NX_REG0[9],goto_table:SNATConntrackZone",
				},
			},
		},
		{
			...
			[]*ofTestUtils.ExpectFlow{
				{
					...
					ActStr:   fmt.Sprintf("set_field:%s->eth_dst,resubmit(,L3DecTTL)", peerNodeMAC.String()),
				},
			},
		})
	}
...
func prepareDefaultFlows(config *testConfig) []expectTableFlows {
	outputStageTable := "L2ForwardingOut"
	ctZone := "4096"
	ctZoneV6 := "12288"
	matchVLANString := ""
	...
	if config.connectUplinkToBridge {
		...
		tableVLANFlows.flows = append(tableVLANFlows.flows, &ofTestUtils.ExpectFlow{MatchStr: "priority=0", ActStr: "goto_table:L2ForwardingOut"})
	...
	tableL3ForwardingFlows := expectTableFlows{
		"L3Forwarding",
		[]*ofTestUtils.ExpectFlow{{MatchStr: "priority=0", ActStr: "load:0x1->NXM_NX_REG0[4..7],goto_table:L2ForwardingCalc"}},
	}
	...
	if config.enableIPv4 {
		...
		tableSNATConntrackCommitFlows.flows = append(tableSNATConntrackCommitFlows.flows,
			&ofTestUtils.ExpectFlow{
				MatchStr: "priority=200,ct_state=+new+trk,ct_mark=0x40/0x40,ip,reg0=0x1/0xf",
			...
			&ofTestUtils.ExpectFlow{
				MatchStr: "priority=190,ct_state=+new+trk,ct_mark=0x20/0x20,ip,reg0=0x1/0xf",
		...
		tableServiceMarkFlows.flows = append(tableServiceMarkFlows.flows,
			&ofTestUtils.ExpectFlow{MatchStr: "priority=200,ct_state=+new+trk,ip,reg0=0x11/0xff", ActStr: "ct(commit,table=SNATConntrackCommit,zone=65520,exec(load:0x1->NXM_NX_CT_MARK[5],load:0x1->NXM_NX_CT_MARK[6]))"},
			&ofTestUtils.ExpectFlow{MatchStr: "priority=190,ct_state=+new+trk,ip,reg0=0x1/0xf,reg4=0x200000/0x200000", ActStr: "ct(commit,table=SNATConntrackCommit,zone=65520,exec(load:0x1->NXM_NX_CT_MARK[5]))"},
		)
		tableL3DecTTLFlows.flows = append(tableL3DecTTLFlows.flows,
			&ofTestUtils.ExpectFlow{MatchStr: "priority=210,ip,reg0=0x1/0xf", ActStr: "goto_table:ServiceMark"},
	...
	if config.enableIPv6 {
		...
		tableConntrackCommitFlows.flows = append(tableConntrackCommitFlows.flows,
			&ofTestUtils.ExpectFlow{MatchStr: "priority=210,ct_mark=0x10/0x10,ipv6", ActStr: "goto_table:L2ForwardingOut"},
			&ofTestUtils.ExpectFlow{MatchStr: "priority=200,ct_state=+new+trk,ipv6", ActStr: fmt.Sprintf("ct(commit,table=L2ForwardingOut,zone=%s,exec(move:NXM_NX_REG0[0..3]->NXM_NX_CT_MARK[0..3]))", ctZoneV6)},
		)
		tableSNATConntrackCommitFlows.flows = append(tableSNATConntrackCommitFlows.flows,
			&ofTestUtils.ExpectFlow{
				MatchStr: "priority=200,ct_state=+new+trk,ct_mark=0x40/0x40,ipv6,reg0=0x1/0xf",
			...
			&ofTestUtils.ExpectFlow{
				MatchStr: "priority=190,ct_state=+new+trk,ct_mark=0x20/0x20,ipv6,reg0=0x1/0xf",
		...
		tableServiceMarkFlows.flows = append(tableServiceMarkFlows.flows,
			&ofTestUtils.ExpectFlow{MatchStr: "priority=200,ct_state=+new+trk,ipv6,reg0=0x11/0xff", ActStr: "ct(commit,table=SNATConntrackCommit,zone=65510,exec(load:0x1->NXM_NX_CT_MARK[5],load:0x1->NXM_NX_CT_MARK[6]))"},
			&ofTestUtils.ExpectFlow{MatchStr: "priority=190,ct_state=+new+trk,ipv6,reg0=0x1/0xf,reg4=0x200000/0x200000", ActStr: "ct(commit,table=SNATConntrackCommit,zone=65510,exec(load:0x1->NXM_NX_CT_MARK[5]))"},
		)
		tableL3DecTTLFlows.flows = append(tableL3DecTTLFlows.flows,
			&ofTestUtils.ExpectFlow{MatchStr: "priority=210,ipv6,reg0=0x1/0xf", ActStr: "goto_table:ServiceMark"},
	...
	tableFlows := []expectTableFlows{
		...
		},
		{
			...
			[]*ofTestUtils.ExpectFlow{
				...
				{MatchStr: "priority=200,reg0=0x10/0xf0", ActStr: "resubmit(,IngressMetric)"},
				{MatchStr: "priority=200,reg0=0/0xf0", ActStr: "resubmit(,IngressMetric)"},
			},
		},
		{
			"IngressRule",
			...
			[]*ofTestUtils.ExpectFlow{
				{
					...
					ActStr:   "resubmit(,EgressMark)",
				},
				{
					MatchStr: fmt.Sprintf("priority=190,ct_state=-rpl+trk,%s,reg0=0/0xf", ipProtoStr),
					ActStr:   fmt.Sprintf("set_field:%s->eth_dst,resubmit(,EgressMark)", gwMACStr),
				},
			},
		},
		{
			...
			[]*ofTestUtils.ExpectFlow{
				...
				{
					MatchStr: "priority=0",
					ActStr:   "load:0x1->NXM_NX_REG0[4..7],goto_table:L2ForwardingCalc",
				},
			},
		},
	}
			...
			[]*ofTestUtils.ExpectFlow{
				{
					...
					ActStr:   fmt.Sprintf("set_field:%s->eth_dst,load:0x%x->NXM_NX_PKT_MARK[0..7],load:0x1->NXM_NX_REG0[4..7],goto_table:L2ForwardingCalc", localGwMAC.String(), mark),
				},
				{
					MatchStr: fmt.Sprintf("priority=200,ct_state=+new+trk,%s,in_port=%d", ipProtoStr, podOFPort),
					ActStr:   fmt.Sprintf("load:0x%x->NXM_NX_PKT_MARK[0..7],load:0x1->NXM_NX_REG0[4..7],goto_table:L2ForwardingCalc", mark),
				},
				{
					MatchStr: fmt.Sprintf("priority=200,%s,in_port=%d", ipProtoStr, podOFPortRemote),
					ActStr:   fmt.Sprintf("set_field:%s->eth_src,set_field:%s->eth_dst,set_field:%s->%s,load:0->NXM_NX_REG0[4..7],goto_table:L2ForwardingCalc", localGwMAC.String(), vMAC.String(), snatIP, tunDstFieldName),
				},
			},
		},
	}
