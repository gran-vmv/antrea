...
func prepareDefaultFlows(config *testConfig) []expectTableFlows {
	...
	if config.enableIPv4 {
		...
		tableConntrackCommitFlows.flows = append(tableConntrackCommitFlows.flows,
			&ofTestUtils.ExpectFlow{MatchStr: "priority=210,ct_mark=0x10/0x10,ip", ActStr: "goto_table:L2ForwardingOut"},
			&ofTestUtils.ExpectFlow{MatchStr: "priority=200,ct_state=+new+trk,ip", ActStr: "ct(commit,table=L2ForwardingOut,zone=65520,exec(move:NXM_NX_REG0[0..3]->NXM_NX_CT_MARK[0..3]))"},
		)
	...
	if config.enableIPv6 {
		...
		tableConntrackCommitFlows.flows = append(tableConntrackCommitFlows.flows,
			...
			&ofTestUtils.ExpectFlow{MatchStr: "priority=200,ct_state=+new+trk,ipv6", ActStr: "ct(commit,table=L2ForwardingOut,zone=65510,exec(move:NXM_NX_REG0[0..3]->NXM_NX_CT_MARK[0..3]))"},
		)
		tableSNATConntrackCommitFlows.flows = append(tableSNATConntrackCommitFlows.flows,
			&ofTestUtils.ExpectFlow{
