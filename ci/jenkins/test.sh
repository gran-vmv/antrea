...
function run_e2e {
    ...
    if [[ $FLEXIBLE_IPAM == true ]]; then
        go test -v -run=TestAntreaIPAM/testAntreaIPAMStatefulSet antrea.io/antrea/test/e2e --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus --antrea-ipam
    else
