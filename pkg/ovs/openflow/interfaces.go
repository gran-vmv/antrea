...
type Bridge interface {
	...
	// by Geneve header with the specified <optClass, optType, optLength>. The value of OptLength must be multiple of 4.
...
type Flow interface {
	OFEntry
	MatchString() string
	// CopyToBuilder returns a new FlowBuilder that copies the matches of the Flow, but does not copy the actions. Reset
	// the priority in the new FlowBuilder if the provided value is not 0.
