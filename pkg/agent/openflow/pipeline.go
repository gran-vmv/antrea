...
var (
	...
	PipelineClassifierTable = newTable("PipelineClassifier", stageStart, pipelineRoot)
	...
	//
	// Before adding a new table here, please read the following instructions carefully.
	//
	...
	// - Choose a stage. All tables in a stage are intended to implement a functional module in pipelines. Existing stages
	//   are defined in file pkg/agent/openflow/feature.go. If you want to add a new stage, please discuss with maintainers
	//   or OVS pipeline developers of Antrea.
	// - Choose a target pipeline. Existing pipelines are defined in file pkg/agent/openflow/feature.go. If you want to add a
	//   new pipeline, please discuss with maintainers or OVS pipeline developers of Antrea.
	// - Decide where to add the new table in the pipeline. The order table declaration decides the order of tables in the
	//   stage. For example:
	//     * If you want to add a table called `DummyTable` between `SpoofGuardTable` and `IPv6Table` in pipelineIP, then
	...
	//          DummyTable       = newTable("Dummy", stageValidation, pipelineIP)
	//          IPv6Table        = newTable("IPv6", stageValidation, pipelineIP)
	//	        IPClassifierTable = newTable("IPClassifier", stageValidation, pipelineIP)
	//       ```
	//      * If you want to add a table called `DummyTable` just before `ARPResponderTable` in pipelineARP, then the table
	//        should be declared before `ARPResponderTable`:
	//       ```go
	//          DummyTable        = newTable("Dummy", stageOutput, binding.PipelineARP)
	//          ARPResponderTable = newTable("ARPResponder", stageOutput, binding.PipelineARP)
	//       ```
	//       * If you want to add a table called `DummyTable` just after `ConntrackStateTable` in pipelineARP, then the
	...
	//          DummyTable          = newTable("Dummy", stageConntrackState, pipelineIP)
	//       ```
	//  - Reference the new table in a feature in file pkg/agent/openflow/feature.go. The table can be referenced by multiple
	...
	// Tables of pipelineARP are declared below. Do don't declare any tables of other pipelines here!
	...
	// Tables of pipelineIP are declared below. Do don't declare any tables of other pipelines here!

	// Tables in stageClassifier:
	ClassifierTable = newTable("Classifier", stageClassifier, pipelineIP)
...
func (c *client) defaultFlows() []binding.Flow {
	...
	for id, pipeline := range c.pipelines {
		...
		case pipelineMulticast:
			...
			// in IPClassifierTable. Note that, IPClassifierTable is in stageValidation of pipeline for IP. In another word,
			// pipelineMulticast is forked from IPClassifierTable in pipelineIP.
			flows = append(flows, multicastPipelineClassifyFlow(cookieID, pipeline))
		}
	}
	// This generates default the flow in PipelineClassifierTable.
...
// Feature: featurePodConnectivity
// Stage: stageConntrackState
// Tables: ConntrackTable, ConntrackStateTable
// Stage: stageConntrack
...
func (f *featureService) conntrackFlows() []binding.Flow {
	...
	for _, ipProtocol := range f.ipProtocols {
		flows = append(flows,
			...
			// Service connection in ConntrackCommitTable again since commit operation in EndpointDNATTable and
...
func (f *featurePodConnectivity) l3FwdFlowToPod(localGatewayMAC net.HardwareAddr,
	...
	for _, ip := range podInterfaceIPs {
		...
		if isAntreaFlexibleIPAM {
			...
			//   - Local Pod --> Local Antrea Pod
			//      * +trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			//      * +trk, FromLocalRegMark, FromLocalCTMark
			//   - Local Antrea Pod --> Local Antrea Pod
			//      * +trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark, AntreaFlexibleIPAMRegMark, HairpinCTMark (if hairpin)
			//      * +trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, AntreaFlexibleIPAMRegMark
			//   - Local Gateway / External (via OVS gateway port) --> Local Antrea Pod
			//      * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
			//   - Remote Pod / Remote Antrea Pod (via OVS uplink port) --> Local Antrea Pod
			//      * +rpl+trk, FromUplinkRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			//      * -rpl+trk, FromUplinkRegMark, FromLocalCTMark
			//      * +rpl+trk, FromUplinkRegMark, FromUplinkCTMark
			//   - Local Node (via OVS bridge local port) --> Local Antrea Pod
			//      * +rpl+trk, FromBridgeRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			//      * -rpl+trk, FromBridgeRegMark, FromBridgeCTMark
			//   - External (via OVS uplink port) --> Local Antrea Pod
			//      * -rpl+trk, FromUplinkRegMark, FromUplinkCTMark
			//      * +rpl+trk, FromUplinkRegMark, FromLocalCTMark
			...
			// Regardless of whether the traffic mode is encap or noEncap, the corresponding traffic models are:
			//   - Local Pod --> Local Pod
			//      * +trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark, HairpinCTMark (if hairpin)
			//   - Local Gateway / Local Node / External (via OVS gateway port) --> Local Pod
			//      * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
			//      * +rpl+trk, FromGatewayRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			// When the traffic mode is encap, the unique corresponding traffic models are:
			//   - Remote Pod (via OVS tunnel port) --> Local Pod
			//      * +rpl+trk, FromTunnelRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			//      * +rpl+trk, FromTunnelRegMark, FromLocalCTMark, RewriteMACRegMark
			//      * -rpl+trk, FromTunnelRegMark, FromTunnelCTMark, RewriteMACRegMark
			// When the traffic mode is noEncap, the unique corresponding traffic models are:
			//   - Local Antrea Pod --> Local Pod
			//      * +trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark, AntreaFlexibleIPAMRegMark
			//      * +trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, AntreaFlexibleIPAMRegMark
			//   - Remote Pod / Remote Antrea Pod (via OVS gateway port) --> Local Pod
			//      * +rpl+trk, FromGatewayRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
...
func (f *featurePodConnectivity) l3FwdFlowToGateway() []binding.Flow {
	...
	for ipProtocol, gatewayIP := range f.gatewayIPs {
		flows = append(flows,
			...
			// Regardless of whether the traffic mode is encap or noEncap, the corresponding traffic model is:
			//	 - Local Pod --> Local Gateway
			//      * -rpl+trk, FromLocalRegMark, FromLocalCTMark
			// The traffic mode is only encap, and the corresponding traffic model is:
			//	 - Remote Pod (via OVS tunnel port) --> Local Gateway
			//      * -rpl+trk, FromTunnelRegMark, FromTunnelCTMark, RewriteMACRegMark
			...
			// Regardless of whether the traffic mode is encap or noEncap, the corresponding traffic models are:
			//   - Local Pod --> Local Gateway
			//      * +rpl+trk, FromLocalRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
			//      * +rpl+trk, FromLocalRegMark, FromGatewayCTMark
			//   - Local Node / External (via OVS gateway port) --> Local Gateway / External (via OVS gateway port)
			//      * +rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark, HairpinCTMark
			// When the traffic mode is encap, the unique corresponding traffic models are:
			//   - Remote Pod (via OVS tunnel port) --> Gateway
			//      * +rpl+trk, FromTunnelRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
			//      * +rpl+trk, FromTunnelRegMark, FromGatewayCTMark
			// When the traffic mode is noEncap, the unique corresponding traffic models are:
			//   - Remote Pod / Remote Antrea Pod (via OVS gateway port) --> Local Gateway
			//      * +rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark, HairpinCTMark
			//   - Local Antrea Pod --> Local Gateway
			//      * +rpl+trk, FromLocalRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark, AntreaFlexibleIPAMRegMark
			L3ForwardingTable.ofTable.BuildFlow(priorityHigh).
				Cookie(cookieID).
				MatchProtocol(ipProtocol).
...
func (f *featurePodConnectivity) l3FwdFlowToRemoteViaTun(localGatewayMAC net.HardwareAddr, peerSubnet net.IPNet, tunnelPeer net.IP) binding.Flow {
	...
	// The traffic mode is only encap, and corresponding traffic models are:
	//   - Local Pod --> Remote Pod
	//     * -rpl+trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
	//     * -rpl+trk, FromLocalRegMark, FromLocalCTMark
	//     * +rpl+trk, FromLocalRegMark, FromTunnelCTMark
	//   - Local Gateway --> Remote Pod
	//     * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
	//     * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark
	//     * +rpl+trk, FromGatewayRegMark, FromTunnelCTMark
	//   - External (via OVS gateway port) --> Remote Pod
	//     * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
	//     * +rpl+trk, FromGatewayRegMark, FromTunnelCTMark (if Egress is enabled)
...
func (f *featurePodConnectivity) l3FwdFlowToRemoteViaGW(localGatewayMAC net.HardwareAddr, peerSubnet net.IPNet) binding.Flow {
	...
	//     * -rpl+trk, FromLocalRegMark, FromLocalCTMark
	//     * +rpl+trk, FromLocalRegMark, FromGatewayCTMark
	//   - Local Gateway / External (via OVS gateway port) --> Remote Pod
	//     * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark, HairpinCTMark
	fb := L3ForwardingTable.ofTable.BuildFlow(priorityNormal).
		Cookie(cookieID).
		MatchProtocol(ipProtocol).
...
func (f *featurePodConnectivity) l3FwdFlowToRemoteViaUplink(remoteGatewayMAC net.HardwareAddr,
	...
	if !isAntreaFlexibleIPAM {
		flows = append(flows,
			...
			//     * -rpl+trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			//     * -rpl+trk, FromLocalRegMark, FromLocalCTMark
			//     * +rpl+trk, FromLocalRegMark, FromUplinkCTMark
			//   - Local Gateway / External (via OVS gateway port) --> Remote Pod
			//     * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark
		...
		// is enabled.
		// The traffic mode is only noEncap, and corresponding traffic models are:
		//   - Local Antrea Pod --> Remote Pod
		//     * -rpl+trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark, AntreaFlexibleIPAMRegMark
...
// Feature: featureNetworkPolicy
// Stage: stageIngressSecurity / stageEgressSecurity
...
// Tables: IngressMetricTable / EgressMetricTable.
// Refactored from:
//   - func (c *client) conjunctionActionDenyFlow(conjunctionID uint32, table binding.Table, priority *uint16, disposition uint32, enableLogging bool) binding.
...
// Feature: featureEgress
// Stage: stageRouting
// Tables: EgressMarkTable
// Refactored from:
...
// Feature: featureEgress
// Stage: stageRouting, EgressMarkTable
// Refactored from:
	...
	for _, ipProtocol := range f.ipProtocols {
		flows = append(flows,
			...
			L3ForwardingTable.ofTable.BuildFlow(priorityLow).
				...
				MatchCTStateTrk(true).
				MatchRegMark(FromLocalRegMark).
				Action().ResubmitToTables(EgressMarkTable.GetID()).
				Done(),
			...
			EgressMarkTable.ofTable.BuildFlow(priorityHigh).
				Cookie(cookieID).
				MatchProtocol(ipProtocol).
				MatchDstIP(f.nodeIPs[ipProtocol]).
				Action().LoadRegMark(ToGatewayRegMark).
				Action().GotoStage(stageSwitching).
				Done(),
		)
...
func generatePipeline(pipelineID binding.PipelineID, requiredTables []*Table) binding.Pipeline {
	...
	if PipelineClassifierTable.ofTable == nil {
		PipelineClassifierTable.ofTable = binding.NewOFTable(binding.NextTableID(),
			PipelineClassifierTable.name,
			PipelineClassifierTable.stage,
			PipelineClassifierTable.pipeline)
	}
...
// realizePipelines sets nextID and missing action for every flow table in every pipeline and realize it on OVS bridge.
func (c *client) realizePipelines() {
	...
	// Realize pipelines in OVS in the order of pipeline ID definition.
	for p := firstPipeline; p <= lastPipeline; p++ {
		if pipeline, ok := c.pipelines[p]; ok {
...
// Feature: featurePodConnectivity
// Stage: stageValidation
// Tables: ClassifierTable, SpoofGuardTable
// Stage: stageConntrackState
...
func (f *featurePodConnectivity) l3FwdFlowToLocalPodCIDR() []binding.Flow {
	...
	for ipProtocol, cidr := range f.localCIDRs {
		...
		//     * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, sources only include Local Gateway
		// When the traffic mode is noEncap, the unique corresponding traffic models are:
...
func (f *featurePodConnectivity) l3FwdFlowToExternal() binding.Flow {
	// Regardless of whether the traffic mode is encap or noEncap, the corresponding traffic models are:
	//   - Pod --> External / Local Node (via OVS gateway port)
	//      * -rpl+trk, FromLocalRegMark, FromLocalCTMark (if Egress is disabled)
	// When the traffic mode is noEncap, the unique corresponding traffic models are:
	//   - Pod --> Remote Antrea Pod (via OVS gateway port)
	//      * -rpl+trk, FromLocalRegMark, FromLocalCTMark
	//   - Antrea Pod --> Remote Antrea Pod / External (via OVS uplink port)
	//      * -rpl+trk, FromLocalRegMark, FromLocalCTMark, AntreaFlexibleIPAMRegMark
...
func (f *featureService) l3FwdFlowsToExternal() []binding.Flow {
	...
	if f.connectUplinkToBridge {
		flows = append(flows,
			// Regardless of whether the traffic mode is encap or noEncap, the corresponding traffic models are:
			//   - Local Pod --> External / Local Node (via OVS gateway port)
			//      * -rpl+trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark (If Egress is disabled)
			//   - External / Local Gateway (via OVS gateway port) --> External / Local Node (via OVS gateway port)
			//      * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark, HairpinCTMark
			// When the traffic mode is noEncap, the unique corresponding traffic models are:
			//   - Local Pod --> Remote Antrea Pod (via OVS gateway port)
			//      * -rpl+trk, FromLocalRegMark, FromLocalCTMark, RewriteMACRegMark, ServiceCTMark
			//   - Local Gateway --> Remote Antrea Pod (via OVS gateway port)
			//      * -rpl+trk, FromGatewayRegMark, FromGatewayCTMark, RewriteMACRegMark, ServiceCTMark, HairpinCTMark
			L3ForwardingTable.ofTable.BuildFlow(priorityLow).
				Cookie(cookieID).
				MatchRegMark(RewriteMACRegMark).
