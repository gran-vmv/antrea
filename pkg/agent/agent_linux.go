...
// prepareOVSBridge returns immediately on Linux if enableBridge is false.
func (i *Initializer) prepareOVSBridge() error {
	...
	uplinkNetConfig.IP = i.nodeConfig.NodeIPAddr
	...
	if _, err = i.ovsBridgeClient.GetOFPort(brName); err == nil {
		...
		if uplinkNetConfig.Name == brName {
			_, adapter, err := util.GetIPNetDeviceFromIP(util.UplinkReservedIPNet.IP)
			if err != nil {
				klog.Errorf("Cannot find uplink device from UplinkReservedIPNet.")
				return err
			}
	...
	if err = util.ConfigureLinkAddresses(uplinkNetConfig.Index, []*net.IPNet{util.UplinkReservedIPNet}); err != nil {
		return err
	}
	...
	// TODO(gran): check no-flood config
	return nil
}
...
// RestoreOVSBridge returns immediately on Linux if enableBridge is false.
// OVS is managed by Antrea in Linux, network config must be restored to uplink before Antrea shutdown.
func (i *Initializer) RestoreOVSBridge() {
	if !i.enableBridge {
		return
	}
	klog.Infof("Restore bridge config to uplink...")
	uplinkNetConfig := i.nodeConfig.UplinkNetConfig
	uplink := uplinkNetConfig.Name
	...
	if uplink != "" {
		...
		if err := util.RemoveInterfaceAddress(uplink, util.UplinkReservedIPNet); err != nil {
			klog.Errorf("Remove reserved IP from %s failed, err: %+v", uplink, err)
		}
	}
	if brLink != nil {
