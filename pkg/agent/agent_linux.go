...
func (i *Initializer) prepareOVSBridge() error {
	...
	if _, err = i.ovsBridgeClient.GetOFPort(brName, false); err == nil {
		...
		if uplinkNetConfig.Name == brName {
			...
			i.nodeConfig.UplinkNetConfig.OFPort = uint32(uplinkPort.OFPort)
			uplinkNetConfig.Name = adapter.Name
			uplinkNetConfig.Index = adapter.Index
	...
	if i.nodeConfig.UplinkNetConfig.OFPort == 0 {
		freePort, err := i.getFreeOFPort()
		if err != nil {
			klog.ErrorS(err, "Failed to find a free port on OVS")
			return err
		}
		i.nodeConfig.UplinkNetConfig.OFPort = uint32(freePort)
